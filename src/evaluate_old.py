...
def get_loss(output, truth):
    if flags.translate_loss_func == "mse":
        return ( (output[...,:54]-truth)**2 ).mean()
    if flags.translate_loss_func == "mae":
        inner = (output[...,:54] - truth) ** 2
        inner = torch.sum(inner, 2)
        inner = torch.sqrt(inner)
        inner = torch.mean(inner)
        return inner
      ...
      if current_step % args.test_every == 0:
        ...
        for action in actions:
          ...
          for steps in range(2, 16, 2):
              ...
              mle_exp = 0#get_loss(preds_exp, decoder_out_exp)/model.batch_size
